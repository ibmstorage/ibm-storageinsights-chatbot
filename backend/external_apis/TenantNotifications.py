...
class TenantNotifications(API):
    name = TENANT_NOTIFICATIONS
    parameters = {TENANT_ID: str}
    description = (
        "Get notifications for a specific tenant based on severity and duration"
    )

    @classmethod
    def _invoke_and_validate(cls, base_url, parameters, headers, logger):
        get_url = f"{base_url}tenants/{parameters[TENANT_ID]}/notifications"
    ...
    @classmethod
    def invoke(cls, parameters):
        response = cls._invoke_and_validate(None, parameters)
        return response
